...
public class ClockwiseexerciseApplication {
	static Pair<List<Set<Integer>>, Integer> bestMeetingsWithSize3(List<Set<Integer>> meetings) {
		Map<Integer, List<Set<Integer>>> histogram = new HashMap<>();
		meetings.forEach(meeting -> {
			for (Integer attendance: meeting) {
				List<Set<Integer>> appears = histogram.get(attendance);
				if (appears == null) {
					appears = new ArrayList<>();
					histogram.put(attendance, appears);
				}
				appears.add(meeting);
			}
		});
		...
		histogram.entrySet().stream().sorted(new Comparator<Map.Entry<Integer, List<Set<Integer>>>>() {
			@Override
			public int compare(Map.Entry<Integer, List<Set<Integer>>> o1, Map.Entry<Integer, List<Set<Integer>>> o2) {
				return Integer.valueOf(o2.getValue().size()).compareTo(o1.getValue().size());
			}
		}).forEach(entry -> {
			List<Set<Integer>> appears = entry.getValue();
			Collections.sort(appears, new Comparator<Set<?>>() {
				@Override
				public int compare(Set<?> o1, Set<?> o2) {
					return Integer.valueOf(o2.size()).compareTo(o1.size());
				}
			});
			appears.forEach(meeting -> {
	...
	static class Meeting {
		Set<Integer> attendances;
		Set<Integer> lostAttendances;
		int diff;

		public Meeting(Set<Integer> attendances, Set<Integer> lostAttendances) {
			this.attendances = attendances;
			this.lostAttendances = lostAttendances;
			this.diff = attendances.size() - lostAttendances.size();
		}
		...
		public String toString() {
			return MoreObjects.toStringHelper(this)
							...
							.add("lostAttendances", lostAttendances)
							.add("diff", diff)
							.toString();
		}
	}
	...
	static Pair<List<Set<Integer>>, Integer> bestMeetingsWithSize2(List<Set<Integer>> inputs) {
		...
		List<Meeting> meetings = inputs.stream()
						.map(target ->new Meeting(target, findLostAttendances(inputs, target))
						).sorted(new Comparator<Meeting>() {
							@Override
							public int compare(Meeting o1, Meeting o2) {
								return Integer.valueOf(o2.diff).compareTo(o1.diff);
							}
	...
	static Pair<List<Set<Integer>>, Integer> tryToFindBestMeetingsWithSizeBySortedMeetings2(List<Meeting> meetings) {
		List<Meeting> bestMeetings = new ArrayList<>();
		Set<Integer> allAttendees = new HashSet<>();
		for (Meeting meeting: meetings) {
			if (Collections.disjoint(meeting.attendances, allAttendees)) {
				bestMeetings.add(meeting);
				allAttendees.addAll(meeting.attendances);
			}
		}

		return new Pair(bestMeetings.stream().map(m -> m.attendances).collect(Collectors.toList()), allAttendees.size());
	}

	static Pair<List<Set<Integer>>, Integer> bestMeetingsWithSize1(List<Set<Integer>> meetings) {
		...
		Pair<List<Set<Integer>>, Integer> best = tryToFindBestMeetingsWithSizeBySortedMeetings1(meetings);
		int aveSize = meetings.stream().mapToInt(Set::size).sum()/meetings.size();
		//System.out.println("aveSize = " + aveSize);
		for (Set<Integer> meeting: meetings) {
			if (meeting.size() > aveSize) {
				System.out.println("try remove big meeting: " + meeting);
				List<Set<Integer>> filteredMeetings = meetings.stream()
								.filter(m -> m != meeting)
								.collect(Collectors.toList());
				Pair<List<Set<Integer>>, Integer> maybeBetter = tryToFindBestMeetingsWithSizeBySortedMeetings1(filteredMeetings);
	...
	static Pair<List<Set<Integer>>, Integer> tryToFindBestMeetingsWithSizeBySortedMeetings1(List<Set<Integer>> meetings) {
		List<Set<Integer>> bestMeetings = new ArrayList<>();
		Set<Integer> allAttendees = new HashSet<>();
		for (Set<Integer> meeting: meetings) {
			if (Collections.disjoint(meeting, allAttendees)) {
		...
		Pair<List<Set<Integer>>, Integer> ret = new Pair(bestMeetings, allAttendees.size());
		//printBestMeetings("try", ret);
		return ret;
	}
	...
	public static void main(String[] args) {
		...
		printBestMeetings("solution 1", bestMeetingsWithSize1(input1));
		printBestMeetings("solution 2", bestMeetingsWithSize2(input1));
		printBestMeetings("solution 3", bestMeetingsWithSize3(input1));

		//[[0,1,2],[2,3]]
		List<Set<Integer>> input2 = Arrays.asList(ImmutableSet.of(0, 1, 2), ImmutableSet.of(2, 3));
		printMeetings("input2", input2);
		printBestMeetings("solution 1", bestMeetingsWithSize1(input2));
		printBestMeetings("solution 2", bestMeetingsWithSize2(input2));
		printBestMeetings("solution 3", bestMeetingsWithSize3(input2));
		...
		printBestMeetings("solution 1", bestMeetingsWithSize1(input3));
		printBestMeetings("solution 2", bestMeetingsWithSize2(input3));
		printBestMeetings("solution 3", bestMeetingsWithSize3(input3));
		...
		printBestMeetings("solution 1", bestMeetingsWithSize1(input4));
		printBestMeetings("solution 2", bestMeetingsWithSize2(input4));
		printBestMeetings("solution 3", bestMeetingsWithSize3(input4));
   ...
   //[[9,16,34],[10,13,18,20,23,28,30,31,32],[4,7,8,11,16,17,19,25,29,36,37],[0,2,23,28],[1,3,7,8,14,15,19,24,25,26,32],[12,28],[16,21,24,33,34],[5,6,10,15,16,17,21,22,24,27,33,34,35]]
		...
		printBestMeetings("solution 1", bestMeetingsWithSize1(input5));
		printBestMeetings("solution 2", bestMeetingsWithSize2(input5));
		printBestMeetings("solution 3", bestMeetingsWithSize3(input5));
		...
		printBestMeetings("solution 1", bestMeetingsWithSize1(input6));
		printBestMeetings("solution 2", bestMeetingsWithSize2(input6));
		printBestMeetings("solution 3", bestMeetingsWithSize3(input6));
		...
		printBestMeetings("solution 1", bestMeetingsWithSize1(input7));
		printBestMeetings("solution 2", bestMeetingsWithSize2(input7));
		printBestMeetings("solution 3", bestMeetingsWithSize3(input7));
	}
